...
* Migrating from vim                                        :TOC_4_gh:noexport:
 ...
 - [[#philosophy哲学][Philosophy(哲学）]]
 - [[#basic-orientation][Basic orientation]]
   - [[#terms条款][Terms(条款)]]
     - [[#modes-vs-states][Modes vs. States]]
     - [[#layers][Layers]]
     - [[#transient-states][Transient-states]]
   - [[#keybinding-conventions约定][Keybinding conventions约定]]
   ...
   - [[#exploring][Exploring]]
 - [[#customization定制][Customization定制]]
...
* Philosophy(哲学）
 One misconception(误解） many vim users have is that Spacemacs is an Emacs /clone/ of
 vim. Spacemacs does not seek(寻求） to completely mimic(模仿) the behavior of vim
...
* Basic orientation
** Terms(条款)
Spacemacs uses some different terminology(术语) than vim, which can cause confusion(混淆)
...
manipulate(操作） text. In Emacs, we have [[./DOCUMENTATION.org#states][states]]. These are equivalent等效 to vim modes.
...
determines决定了 the editing behavior of Emacs in the current buffer. There is
generally a corresponding相应的 =major-mode= per filetype. An example of a =major-mode= is
=python-mode=, which provides python specific settings in python files. There is
only one =major-mode= per buffer.

*** Layers
Spacemacs has the concept of layers.(层的概念) Layers are similar to vim plugins. They
provide new features to use in Spacemacs. However, layers are often comprised(组成）of
...
[[./DOCUMENTATION.org#configuration-layers][documentation]]. There is also a more in-depth深入 guide on writing layers [[file:LAYERS.org][here]].

*** Transient-states
Spacemacs provides a special functionality功能 called transient-states. Transient-states
allow similar commands to be run in succession without repeatedly pressing the
~<Leader>~ key. Transient-states are usually triggered触发 by using a keybinding with the
...
Note: You can run Emacs interactive交互式 commands using ~:~, but you cannot run ex
...
file will be created with the appropriate适当的 style selected. Most trivial
